...
import type { AnchorState } from './AnchorSettings.vue';
import type { ResizeState } from './ResizeSettings.vue';
...
const getDrawData = (width: number, height: number, anchorState: AnchorState) => {
  // キャンバスサイズ、オフセットを計算
...
const updateBaseImage = (image: HTMLImageElement | null) => {
  ...
  if (image) {
    baseResize.value.widthPixels = image.width;
    baseResize.value.heightPixels = image.height;
  }
};
const updateGlowImage = (image: HTMLImageElement | null) => {
  ...
  if (image) {
    glowResize.value.widthPixels = image.width;
    glowResize.value.heightPixels = image.height;
  }
};
...
// 設定値
const baseResize = ref<ResizeState>({
  sizeType: 'block',
  keepAspect: true,
  sizePriority: 'width',
  widthPixels: 9,
...
const baseAnchor = ref<AnchorState>({
  anchorPosition: 'center',
  offsetX: 0,
  offsetY: 0,
});

const glowResize = ref<ResizeState>({
  sizeType: 'block',
  keepAspect: true,
  sizePriority: 'width',
  widthPixels: 9,
...
const glowAnchor = ref<AnchorState>({
  anchorPosition: 'center',
  offsetX: 0,
  offsetY: 0,
});
...
const baseDrawData = computed(() => getDrawData(baseResize.value.widthPixels, baseResize.value.heightPixels, baseAnchor.value));
const glowDrawData = computed(() => {
  const { x, y } = getAnchorOffset(glowResize.value.widthPixels, glowResize.value.heightPixels, baseResize.value.widthPixels, baseResize.value.heightPixels, glowAnchor.value.anchorPosition);
  const v = {
    offsetX: x + glowAnchor.value.offsetX + baseDrawData.value.offsetX,
    offsetY: y + glowAnchor.value.offsetY + baseDrawData.value.offsetY,
  };
...
const marginExists = computed(() => {
  ...
  return offsetX !== 0 || offsetY !== 0 || canvasWidth !== baseResize.value.widthPixels || canvasHeight !== baseResize.value.heightPixels;
});

watch([imageSizeMatched, baseResize], () => {
  if (imageSizeMatched.value) {
    glowResize.value = { ...baseResize.value };
  }
}, { deep: true });
...
// リサイズ描画処理
const drawResizedImage = (ctx: CanvasRenderingContext2D, img: HTMLImageElement | HTMLCanvasElement, resizeState: ResizeState, drawData: { offsetX: number; offsetY: number }) => {
...
watch(
  [baseImage, glowImageDisplay, baseResize, baseDrawData, glowResize, glowDrawData, bgColor, showGlow],
  () => {
...
const paintCanvas = (baseCanvas: HTMLCanvasElement, glowCanvas?: HTMLCanvasElement, displayMode?: boolean) => {
  ...
  drawResizedImage(bCtx, baseImage.value, baseResize.value, baseDrawData.value);
  ...
  if (displayMode) {
    if (glowImageDisplay.value) {
      ...
      drawResizedImage(bCtx, glowImageDisplay.value, glowResize.value, glowDrawData.value);
      drawResizedImage(gCtx, glowImageDisplay.value, glowResize.value, glowDrawData.value);
    }
  }
  else if (glowImage.value) {
    drawResizedImage(gCtx, glowImage.value, glowResize.value, glowDrawData.value);
  }
};
