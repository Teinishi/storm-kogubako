...
async function saveVehicleXml() {
  ...
  const xml = generatePaintableSignVehicle(width, height, baseImageData.data, glowImageData?.data, {
...
<template>
  <div class="grid h-full sm:grid-cols-[24rem_1fr]">
    ...
    <ResponsivePanel :label="t('preview')" icon="i-lucide-eye" :disabled="!state.baseImage">
      <ClientOnly>
        <TheImg2PaintPreview
